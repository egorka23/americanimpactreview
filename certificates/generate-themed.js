const puppeteer = require('puppeteer');
const fs = require('fs');
const path = require('path');

const articles = [
  {
    id: 'e2026001',
    title: 'Monitoring and Scalability of High-Load Systems: An Evidence-Based Framework for Real-Time SLA Compliance and Customer Satisfaction Optimization',
    authors: ['Nikolai Stepanov', 'Bogdan Mikhaylov'],
    received: 'January 11, 2026',
    published: 'February 10, 2026',
    doi: '10.70729/air.2026.0201',
  },
  {
    id: 'e2026002',
    title: 'Diagnostic Capabilities of Hardware-Software Systems in Sports Medicine',
    authors: ['V.M. Alekseev'],
    received: 'January 11, 2026',
    published: 'February 10, 2026',
    doi: '10.70729/air.2026.0202',
  },
  {
    id: 'e2026003',
    title: 'Finger Dermatoglyphics as Predictive Markers of Physical Abilities: Applications in Athlete Selection and Training',
    authors: ['Tamara F. Abramova', 'Tatyana M. Nikitina', 'Nadezhda I. Kochetkova'],
    received: 'January 11, 2026',
    published: 'February 10, 2026',
    doi: '10.70729/air.2026.0203',
  },
  {
    id: 'e2026004',
    title: 'Laboratory Assessment of Aerobic and Anaerobic Performance in Elite Greco-Roman Wrestlers: A Case Series Using the Wingate Anaerobic Test and Graded Exercise Testing',
    authors: ['Ivan Timme'],
    received: 'January 15, 2026',
    published: 'February 10, 2026',
    doi: '10.70729/air.2026.0204',
  },
  {
    id: 'e2026005',
    title: 'Genetic Markers for Talent Identification and Training Individualization in Elite Combat Sport and Endurance Athletes: Insights from a National Sports Science Program',
    authors: ['Roman Andreev'],
    received: 'January 15, 2026',
    published: 'February 10, 2026',
    doi: '10.70729/air.2026.0205',
  },
  {
    id: 'e2026006',
    title: 'Longitudinal Physiological Monitoring and Evidence-Based Training Periodization in Junior Cross-Country Skiers: A Three-Season Case Study',
    authors: ['Alex Ver'],
    received: 'January 20, 2026',
    published: 'February 10, 2026',
    doi: '10.70729/air.2026.0206',
  },
  {
    id: 'e2026007',
    title: 'Leveraging Artificial Intelligence for Scalable Customer Success in Mobile Marketing Technology: A Systematic Review and Strategic Framework',
    authors: ['Eugene Mishchenko', 'Irina Smirnova'],
    received: 'January 20, 2026',
    published: 'February 11, 2026',
    doi: '10.70729/air.2026.0207',
  },
];

// Theme 1: Navy + Gold (like variant #1 from the PDF)
const THEME_NAVY = {
  name: 'navy',
  bgColor: '#0c1a3a',
  bgGradient: 'radial-gradient(ellipse at 50% 40%, #162a55 0%, #0c1a3a 70%)',
  borderColor: '#c9a020',
  borderColorLight: 'rgba(201,160,32,0.3)',
  textPrimary: '#e8d5a0',
  textSecondary: 'rgba(232,213,160,0.7)',
  textMuted: 'rgba(232,213,160,0.5)',
  titleBorder: 'rgba(201,160,32,0.4)',
  starColor: '#c9a020',
  hrGradient: 'linear-gradient(90deg, transparent, #c9a020, transparent)',
  sigColor: '#e8d5a0',
  issnColor: 'rgba(232,213,160,0.6)',
  cornerColor: '#c9a020',
};

// Theme 3: Maroon/Dark Burgundy + Gold (like variant #3 from the PDF)
const THEME_MAROON = {
  name: 'maroon',
  bgColor: '#1a0a0a',
  bgGradient: 'radial-gradient(ellipse at 50% 40%, #3a1515 0%, #1a0a0a 70%)',
  borderColor: '#c9a020',
  borderColorLight: 'rgba(201,160,32,0.3)',
  textPrimary: '#e8d5a0',
  textSecondary: 'rgba(232,213,160,0.7)',
  textMuted: 'rgba(232,213,160,0.5)',
  titleBorder: 'rgba(201,160,32,0.4)',
  starColor: '#c9a020',
  hrGradient: 'linear-gradient(90deg, transparent, #c9a020, transparent)',
  sigColor: '#e8d5a0',
  issnColor: 'rgba(232,213,160,0.6)',
  cornerColor: '#c9a020',
};

function adaptFontSizes(titleLen, nameLen) {
  let titleSize, nameSize;
  if (titleLen <= 60) titleSize = 28;
  else if (titleLen <= 100) titleSize = 24;
  else if (titleLen <= 150) titleSize = 21;
  else if (titleLen <= 200) titleSize = 18;
  else titleSize = 16;

  if (nameLen <= 15) nameSize = 52;
  else if (nameLen <= 25) nameSize = 44;
  else if (nameLen <= 35) nameSize = 38;
  else nameSize = 32;

  return { titleSize, nameSize };
}

function buildHTML(article, authorName, theme) {
  const { titleSize, nameSize } = adaptFontSizes(article.title.length, authorName.length);
  const T = theme;

  const sigPath = path.join(__dirname, 'signature-gold.svg');
  let sigTag = '';
  if (fs.existsSync(sigPath)) {
    sigTag = `<img class="sig-svg" src="file://${sigPath}" alt="Egor Akimov, Ph.D.">`;
  } else {
    sigTag = `<div style="font-family:'Great Vibes',cursive;font-size:36px;color:${T.textPrimary};">Egor Akimov</div>`;
  }

  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<style>
  @import url('https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;0,700;0,900;1,400;1,700&family=Cormorant+Garamond:ital,wght@0,400;0,600;0,700;1,400;1,600&family=Great+Vibes&family=EB+Garamond:ital,wght@0,400;0,500;0,600;1,400;1,500&display=swap');
  @font-face {
    font-family: 'Amsterdam';
    src: url('file://${path.join(__dirname, 'Amsterdam.ttf')}') format('truetype');
  }
  * { margin: 0; padding: 0; box-sizing: border-box; }
  @page { size: 816px 1056px; margin: 0; }
  html, body {
    width: 816px; height: 1056px;
    font-family: 'EB Garamond', serif;
    background: ${T.bgColor};
    overflow: hidden;
    -webkit-print-color-adjust: exact;
    print-color-adjust: exact;
  }

  .frame {
    position: absolute; top: 0; left: 0; width: 816px; height: 1056px; z-index: 1;
  }

  .page {
    position: relative; z-index: 10;
    width: 816px; height: 1056px;
    background: ${T.bgGradient};
    display: flex; flex-direction: column;
    align-items: center; justify-content: space-evenly;
    padding: 55px 90px;
    overflow: hidden;
  }

  .header { text-align: center; }
  .journal-name {
    font-family: 'Playfair Display', serif;
    font-size: 30px; font-weight: 900;
    color: ${T.textPrimary}; letter-spacing: 3px; text-transform: uppercase;
  }
  .journal-subtitle { font-size: 14px; color: ${T.textSecondary}; margin-top: 4px; letter-spacing: 1px; }

  .cert-section { text-align: center; width: 100%; }
  .hr { width: 100%; height: 1px; background: ${T.hrGradient}; }
  .cert-title {
    font-family: 'Playfair Display', serif;
    font-size: 17px; font-weight: 700;
    color: ${T.starColor}; letter-spacing: 5px; text-transform: uppercase;
    margin: 8px 0 4px;
  }
  .star { color: ${T.starColor}; font-size: 16px; }

  .body-section { text-align: center; }
  .certify-text {
    font-family: 'Cormorant Garamond', serif;
    font-size: 18px; font-style: italic; color: ${T.textSecondary}; margin-bottom: 14px;
  }
  .article-title {
    font-family: 'Playfair Display', serif;
    font-weight: 700; color: ${T.textPrimary};
    text-align: center; padding: 12px 24px;
    border-top: 1.5px solid ${T.titleBorder}; border-bottom: 1.5px solid ${T.titleBorder};
    max-width: 600px; line-height: 1.35;
    font-size: ${titleSize}px;
  }
  .written-by {
    font-family: 'Cormorant Garamond', serif;
    font-size: 16px; font-style: italic; color: ${T.textSecondary}; margin-top: 22px; margin-bottom: 16px;
  }
  .author-name {
    font-family: 'Amsterdam', 'Great Vibes', cursive;
    font-size: ${nameSize}px; color: ${T.textPrimary}; line-height: 1.2;
    margin-top: 24px; margin-bottom: 10px;
  }

  .details-section { text-align: center; margin-top: 28px; }
  .details {
    display: inline-block; text-align: left;
    font-size: 17px; color: ${T.textSecondary}; line-height: 1.8;
  }
  .details .row { display: flex; gap: 8px; }
  .details .label { font-weight: 600; text-align: right; min-width: 80px; color: ${T.textPrimary}; }
  .details .sep { color: ${T.starColor}; }
  .details .val { font-style: italic; }

  .peer-section { text-align: center; }
  .peer-text {
    font-family: 'Cormorant Garamond', serif;
    font-size: 17px; font-style: italic; color: ${T.textSecondary}; line-height: 1.5;
  }
  .peer-text .jb { font-weight: 700; font-style: italic; color: ${T.textPrimary}; }

  .footer {
    width: 100%;
    display: flex; justify-content: space-between; align-items: flex-end;
  }
  .sig-block { text-align: left; }
  .sig-svg { width: 240px; height: auto; display: block; margin-bottom: 2px; }
  .sig-title { font-family: 'Cormorant Garamond', serif; font-size: 14px; font-style: italic; color: ${T.textMuted}; }
  .sig-org { font-size: 14px; color: ${T.textPrimary}; font-weight: 600; }

  .seal-block { text-align: center; }
  .seal-img { width: 150px; height: auto; filter: drop-shadow(0 4px 16px rgba(201,160,32,0.3)); }
  .issn { font-size: 13px; color: ${T.issnColor}; margin-top: 4px; font-weight: 600; letter-spacing: 1px; }
</style>
</head>
<body>

<div class="page">
  <!-- Gold double border frame -->
  <svg class="frame" viewBox="0 0 816 1056" xmlns="http://www.w3.org/2000/svg" style="position:absolute;top:0;left:0;width:816px;height:1056px;z-index:2;pointer-events:none;">
    <!-- Outer border -->
    <rect x="18" y="18" width="780" height="1020" rx="4" fill="none" stroke="${T.borderColor}" stroke-width="2"/>
    <!-- Inner border -->
    <rect x="30" y="30" width="756" height="996" rx="3" fill="none" stroke="${T.borderColorLight}" stroke-width="1"/>
    <!-- Corner ornaments - top left -->
    <line x1="18" y1="60" x2="55" y2="60" stroke="${T.borderColor}" stroke-width="1.5" opacity="0.6"/>
    <line x1="60" y1="18" x2="60" y2="55" stroke="${T.borderColor}" stroke-width="1.5" opacity="0.6"/>
    <path d="M 22,22 L 55,22 L 55,28 L 28,28 L 28,55 L 22,55 Z" fill="${T.borderColor}" opacity="0.5"/>
    <!-- Corner ornaments - top right -->
    <line x1="761" y1="60" x2="798" y2="60" stroke="${T.borderColor}" stroke-width="1.5" opacity="0.6"/>
    <line x1="756" y1="18" x2="756" y2="55" stroke="${T.borderColor}" stroke-width="1.5" opacity="0.6"/>
    <path d="M 794,22 L 761,22 L 761,28 L 788,28 L 788,55 L 794,55 Z" fill="${T.borderColor}" opacity="0.5"/>
    <!-- Corner ornaments - bottom left -->
    <line x1="18" y1="996" x2="55" y2="996" stroke="${T.borderColor}" stroke-width="1.5" opacity="0.6"/>
    <line x1="60" y1="1001" x2="60" y2="1038" stroke="${T.borderColor}" stroke-width="1.5" opacity="0.6"/>
    <path d="M 22,1034 L 55,1034 L 55,1028 L 28,1028 L 28,1001 L 22,1001 Z" fill="${T.borderColor}" opacity="0.5"/>
    <!-- Corner ornaments - bottom right -->
    <line x1="761" y1="996" x2="798" y2="996" stroke="${T.borderColor}" stroke-width="1.5" opacity="0.6"/>
    <line x1="756" y1="1001" x2="756" y2="1038" stroke="${T.borderColor}" stroke-width="1.5" opacity="0.6"/>
    <path d="M 794,1034 L 761,1034 L 761,1028 L 788,1028 L 788,1001 L 794,1001 Z" fill="${T.borderColor}" opacity="0.5"/>
  </svg>

  <div class="header" style="position:relative;z-index:5;">
    <div class="journal-name">American Impact Review</div>
    <div class="journal-subtitle">A Peer-Reviewed Multidisciplinary Journal</div>
  </div>

  <div class="cert-section" style="position:relative;z-index:5;">
    <div class="hr"></div>
    <div class="cert-title">Certificate of Publication</div>
    <div class="star">&#9733;</div>
    <div class="hr"></div>
  </div>

  <div class="body-section" style="position:relative;z-index:5;">
    <div class="certify-text">This is to certify that the article entitled</div>
    <div class="article-title">\u201C${article.title}\u201D</div>
    <div class="written-by">authored by</div>
    <div class="author-name">${authorName}</div>
  </div>

  <div class="details-section" style="position:relative;z-index:5;">
    <div class="details">
      <div class="row"><span class="label">Received:</span><span class="sep">|</span><span class="val">${article.received}</span></div>
      <div class="row"><span class="label">Published:</span><span class="sep">|</span><span class="val">${article.published}</span></div>
      <div class="row"><span class="label">DOI:</span><span class="sep">|</span><span class="val">${article.doi}</span></div>
    </div>
  </div>

  <div class="peer-section" style="position:relative;z-index:5;">
    <div class="peer-text">
      has been peer reviewed and published in<br>
      <span class="jb">American Impact Review</span>
    </div>
  </div>

  <div class="footer" style="position:relative;z-index:5;">
    <div class="sig-block">
      ${sigTag}
      <div class="sig-title">Editor-in-Chief</div>
      <div class="sig-org">American Impact Review</div>
    </div>
    <div class="seal-block">
      <img class="seal-img" src="file://${path.join(__dirname, 'seals', 'seal-06.svg')}" alt="AIR Seal">
      <div class="issn">ISSN: 2996-2781</div>
    </div>
  </div>
</div>
</body>
</html>`;
}

(async () => {
  const outDir = '/Users/aeb/Downloads/AIR-Certificates-Themed';
  if (!fs.existsSync(outDir)) fs.mkdirSync(outDir, { recursive: true });

  const browser = await puppeteer.launch({ headless: true });
  let count = 0;

  const themes = [THEME_NAVY, THEME_MAROON];

  for (const theme of themes) {
    const themeDir = path.join(outDir, theme.name);
    if (!fs.existsSync(themeDir)) fs.mkdirSync(themeDir, { recursive: true });

    for (const article of articles) {
      for (const author of article.authors) {
        const html = buildHTML(article, author, theme);
        const tmpFile = path.join(__dirname, `_tmp_cert_${theme.name}.html`);
        fs.writeFileSync(tmpFile, html);

        const page = await browser.newPage();
        await page.goto(`file://${tmpFile}`, { waitUntil: 'networkidle0', timeout: 15000 });
        await new Promise(r => setTimeout(r, 1500));

        const safeName = author.replace(/[^a-zA-Z0-9]/g, '_');
        const filename = `${article.id}-${safeName}.pdf`;
        await page.pdf({
          path: path.join(themeDir, filename),
          width: '816px',
          height: '1056px',
          printBackground: true,
          preferCSSPageSize: true,
        });
        await page.close();
        fs.unlinkSync(tmpFile);
        count++;
        console.log(`[${count}] ${theme.name}/${filename}`);
      }
    }
  }

  await browser.close();
  console.log(`\nDone! ${count} certificates saved to ${outDir}`);
})();
