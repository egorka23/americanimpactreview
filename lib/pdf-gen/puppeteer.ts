/**
 * Puppeteer-based PDF generation (legacy).
 * Generates article PDF via headless Chrome HTML rendering.
 * Extracted from regenerate-pdf/[slug]/route.ts for modularity.
 */
...
export async function generatePdfPuppeteer(r: ArticleRow): Promise<PdfResult> {
  ...
  const articleUrl = `https://americanimpactreview.com/article/${r.slug}`;
